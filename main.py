...
try:
                 ...
                 print (f"Please Install Module First: \033[93m{missing_module}\033[0m")
                 sys.exit()
### 14 modules
...
####################
                      ...
                      number = input(f"{WHITE}Enter Number {NORMAL}({GREEN}Without +){WHITE} :{CRED2}➤ ")
                      ### REQUESTS API
                      phe = requests.get("http://apilayer.net/api/validate?access_key=I2A2HXBuGbEJbufG8AxD2IGeFpqFyWFU=" + number )
                      sleep(1)
                      print ()
                      sleep(0.1)
                      ### track number v2
